...
int vec3_triangle_buffer_clear(VEC3TriangleBuffer *buffer) {
  ...
  if (!vec3_triangle_buffer_is_empty(*buffer)) {
    free(buffer->items);
    buffer->items = 0;
    buffer->length = 0;
  }
...
int vec3_triangle_buffer_allocate(VEC3TriangleBuffer *buffer, int64_t n) {
  VECTOR_BUFFER_GUARD(buffer);
  buffer->items =
      (VEC3Triangle *)realloc(buffer->items, (buffer->length + n) * sizeof(VEC3Triangle));
  return buffer->items != 0;
}


int vec3_triangle_buffer_shift_left(VEC3TriangleBuffer *array, int index) {
...
int vec3_triangle_buffer_shift_right(VEC3TriangleBuffer *array, int index) {
  if (!array || array->length <= 0) return 0;
